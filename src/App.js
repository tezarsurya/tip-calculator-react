...
function App() {
  ...
  const handlePercentButton = (e) => {
    let bill = !isNaN(parseFloat(values.bill)) ? parseFloat(values.bill) : 0;
    let people = !isNaN(parseInt(values.people)) ? parseInt(values.people) : 0;
    let tempTip = (parseFloat(e.target.id) / 100) * bill;
    let tip = (tempTip / people).toFixed(2);
    let total = ((bill + tempTip) / people).toFixed(2);
  ...
  const handleReset = () => {
    ...
    Array.from(forms).map((form) => form.reset());
  };
  ...
  return (
    <div className='App'>
      ...
      <svg id='logo' xmlns='http://www.w3.org/2000/svg' width='87' height='54'>
        <path
          fill='#3D6666'
      ...
      <div className='container'>
        ...
        <Attribution small={true} />
      </div>
      <Attribution small={false} />
    </div>
  );
}
